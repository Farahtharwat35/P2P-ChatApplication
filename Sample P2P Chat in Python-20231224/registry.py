...
class ClientThread(threading.Thread):
    ...
    def _init_(self, ip, port, tcpClientSocket):
        ...
        while True:
            try:
                ...
                #   LOGOUT  #
                    ...
                    if not db.is_room_exits(message[1]):
                    ...
                    # if an account with the username already online
                        ...
                        db.appendmembers(message[1],self.username, message[3],message[4])
                    ...
                    if db.LEAVE_ROOM(message[1],message[2])==1:
                      response = "USER LEFT "
                      logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                      self.tcpClientSocket.send(response.encode())
                    elif db.LEAVE_ROOM(message[1],message[2])==2:
                      response = "USER-NOT-IN-THIS-ROOM "
                      logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                      self.tcpClientSocket.send(response.encode())
                    else:
...
class UDPServer(threading.Thread):
    ...
    def _init_(self, username, clientSocket):
        threading.Thread._init_(self)
